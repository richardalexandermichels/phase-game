...
internal fun patternForJudgmentBar(
    candidateClockBar: Long,
    currentClockBar: Long,
    knownPatternsByClockBar: Map<Long, QueuedPatternBar>,
    patternQueue: List<QueuedPatternBar>,
    currentPerformance: BarPerformance?,
    fallbackRhythm: List<Boolean>
): QueuedPatternBar {
    knownPatternsByClockBar[candidateClockBar]?.let { return it }

    val currentPattern = patternQueue.firstOrNull() ?: QueuedPatternBar(
        absoluteBarIndex = candidateClockBar,
        phaseIndex = 0,
    ...
    val shouldAdvanceForNextBar =
        candidateClockBar == currentClockBar + 1L &&
            isCompletedWithoutMisses(
                performance = currentPerformance,
                activeRhythm = currentPattern.rhythm
            )

    return if (shouldAdvanceForNextBar) {
        patternQueue.getOrNull(1) ?: currentPattern
    } else {
        currentPattern
    }
}
